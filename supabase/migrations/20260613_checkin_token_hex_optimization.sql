-- ============================================================================
-- Optimización de Token QR (Hexadecimal Compacto de 56 Caracteres)
--
-- Reduce la longitud del token de 152 a 56 caracteres, acelerando el tipeo
-- de lectores de código de barras USB emuladores de teclado en un 300%.
-- Mantiene soporte retrocompatible con el formato JWT anterior.
-- ============================================================================

-- 1. EMITIR PASE (cliente autenticado) - Formato Hex de 56 chars -------------
...
begin
  ...
  v_uid_hex := replace(v_uid::text, '-', '');
  v_exp_hex := lpad(to_hex(v_exp), 8, '0');

  -- Firma hmac de la concatenación, codificada a hex y truncada a 16 chars (64 bits)
  v_sig_hex := substring(
  ...
  return jsonb_build_object(
    'token',       v_uid_hex || v_exp_hex || v_sig_hex,
...
-- 2. ESCANEAR PASE (admin) - Soporta ambos formatos --------------------------
